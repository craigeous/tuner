  ...
  <header class="app-header">
    ...
    <div class="header-controls">
      ...
      <div class="control-badge" title="Calibration reference pitch (Standard: 440 Hz)">
        ...
        <span class="value-text" id="a4-display">${C} Hz</span>
      ...
      <div class="control-badge">
        ...
        <select class="app-select" id="notation-select" aria-label="Select musical notation style">
          <option value="sharp" ${S===`sharp`?`selected`:``}>Sharps (♯)</option>
          <option value="flat" ${S===`flat`?`selected`:``}>Flats (♭)</option>
          <option value="solfege" ${S===`solfege`?`selected`:``}>Solfège (Do-Re-Mi)</option>
        </select>
      </div>
      ...
      <div class="preset-card">
        <div class="preset-header">
          <div class="preset-title-wrap">
            <h3 id="preset-title">${x.name}</h3>
            <p id="preset-desc">${x.description}</p>
          </div>
          <select class="app-select" id="preset-select" aria-label="Select instrument tuning or vocal profile">
            ${v.map(e=>`<option value="${e.id}" ${e.id===x.id?`selected`:``}>${e.name}</option>`).join(``)}
          </select>
        </div>
      ...
      <div class="visualizer-card">
        ...
        <div class="audio-meter-bar">
          <div class="meter-row-top">
            ...
            <button type="button" class="btn-boost-quick ${y.getInputGain()>=3?`active`:``}" id="btn-quick-ipad-boost" title="Preamp boost for iPad Pro/Air mic arrays">
          ...
          <div class="meter-controls-grid">
            ...
            <div class="gate-control-wrap" title="Software preamp gain multiplier. Boosts quiet tablet microphone arrays.">
              ...
              <input type="range" id="mic-gain-slider" min="1" max="10" step="0.5" value="${y.getInputGain()}" style="width: 80px;" />
              <span class="value-text" id="mic-gain-label" style="min-width: 42px;">${y.getInputGain().toFixed(1)}x</span>
            </div>
            ...
            <div class="gate-control-wrap" title="Lower values detect softer notes; higher values filter out background noise">
              ...
              <input type="range" id="noise-gate-slider" min="0.0003" max="0.02" step="0.0003" value="${y.getNoiseGate()}" style="width: 80px;" />
              <span class="value-text" id="noise-gate-label" style="min-width: 48px;">${(y.getNoiseGate()*1e3).toFixed(1)}m</span>
            </div>

            <!-- Auto Gain Control Toggle -->
            <div class="gate-control-wrap">
              <label title="Engages hardware automatic gain leveling in iPadOS/browser">
                <input type="checkbox" id="auto-gain-toggle" ${y.getAutoGainControl()?`checked`:``} />
                Auto-Gain (AGC)
              </label>
            </div>
            ...
            <div class="gate-control-wrap">
              <label title="Disables echo cancellation and noise suppression for instruments">
                <input type="checkbox" id="raw-audio-toggle" ${y.getRawAudioMode()?`checked`:``} />
                Raw Musician Audio
              </label>
            </div>
          </div>
      ...
      <div class="tone-card">
        ...
        <div class="tone-playback-bar">
          <button type="button" class="btn-big-play" id="btn-play-tone">
            <span>🔊</span> Play Note (${e[w]}${T} &bull; ${E.toFixed(1)} Hz)
          </button>
        ...
        <div class="tone-pitch-selector-grid">
          ...
          <div class="control-panel-box">
            ...
            <div class="note-grid-12" id="tone-note-grid">
              ${e.map((e,t)=>`<button type="button" class="note-grid-btn ${t===w?`active`:``}" data-idx="${t}">${e}</button>`).join(``)}
            </div>
          </div>
          ...
          <div class="control-panel-box">
            ...
            <div style="display: flex; align-items: center; justify-content: space-between;">
              ...
              <div style="display: flex; align-items: center; gap: 0.5rem;">
                ...
                <span class="value-text" id="gen-oct-display">${T}</span>
            ...
            <div style="margin-top: 0.85rem;">
              <div style="display: flex; justify-content: space-between; font-size: 0.82rem; color: var(--text-muted); margin-bottom: 0.2rem;">
                ...
                <span style="font-family: var(--font-mono); color: var(--accent-cyan); font-weight: 600;" id="gen-freq-display">${E.toFixed(1)} Hz</span>
              </div>
              <input type="range" id="gen-freq-slider" min="50" max="1500" step="0.5" value="${E}" style="width: 100%;" />
            </div>
          </div>
        </div>
      </div>
...
`;var O=new l(document.querySelector(`#tuner-gauge-canvas`)),k=new u(document.querySelector(`#pitch-trace-canvas`)),A=new h(document.querySelector(`#piano-container`),b,{startOctave:3,numOctaves:2,a4:C,notation:S}),te=new _(document.querySelector(`#trainer-container`),b,C),ne=new g(document.querySelector(`#generator-mini-tuner`),{label:`Live Voice / Instrument Pitch Matcher`,showReferenceComparison:!0}),re=new g(document.querySelector(`#piano-mini-tuner`),{label:`Live Microphone Pitch Tracker`,compact:!0}),j=document.querySelector(`#btn-master-mic`),M=document.querySelector(`#mic-status-label`),ie=document.querySelector(`#note-letter`),ae=document.querySelector(`#note-accidental`),oe=document.querySelector(`#note-octave`),se=document.querySelector(`#note-solfege`),N=document.querySelector(`#tuning-guidance`),ce=document.querySelector(`#stat-cents`),le=document.querySelector(`#stat-freq`),ue=document.querySelector(`#stat-target-freq`),de=document.querySelector(`#vu-meter-bar`),P=document.querySelector(`#tuner-hero-card`);y.subscribe(t=>{let n=Math.min(100,Math.round(t.rms*350));de&&(de.style.width=`${n}%`),k.update(t),te.updatePitch(t);let r=`${e[w]}${T}`;if(ne.update(t,E,r),re.update(t),t.note&&t.frequency>0){let e=t.note;ie.textContent=e.noteName[0],ae.textContent=e.accidental,oe.textContent=String(e.octave),se.textContent=`Solfège: ${e.solfege} ${e.octave}`,ce.textContent=`${e.cents>0?`+`:``}${e.cents}¢`,le.textContent=`${e.frequency.toFixed(1)} Hz`,ue.textContent=`${e.targetFrequency.toFixed(1)} Hz`,P.className=`tuner-hero-card`,N.className=`tuning-guidance-badge`,Math.abs(e.cents)<=3?(P.classList.add(`in-tune`),N.classList.add(`in-tune`),N.textContent=`✓ PERFECT IN TUNE`):Math.abs(e.cents)<=8?(P.classList.add(`close-tune`),e.cents<0?(N.classList.add(`flat`),N.textContent=`▲ SLIGHTLY FLAT`):(N.classList.add(`sharp`),N.textContent=`▼ SLIGHTLY SHARP`)):e.cents<0?(P.classList.add(`flat-tune`),N.classList.add(`flat`),N.textContent=`▲ TUNE UP (${Math.abs(e.cents)}¢ FLAT)`):(P.classList.add(`sharp-tune`),N.classList.add(`sharp`),N.textContent=`▼ TUNE DOWN (+${e.cents}¢ SHARP)`),O.update(e),A.highlightMidi(Math.round(e.midi)),he(e.midi,Math.abs(e.cents)<=3)}else O.update(null),A.highlightMidi(null),he(null,!1),y.isListening()?(N.textContent=`Listening for notes...`,N.className=`tuning-guidance-badge`):(N.textContent=`Microphone off. Click Start Listening.`,N.className=`tuning-guidance-badge`)});var fe=document.querySelector(`#preset-select`),F=document.querySelector(`#preset-strings-container`),pe=document.querySelector(`#preset-title`),me=document.querySelector(`#preset-desc`);function I(){if(pe.textContent=x.name,me.textContent=x.description,F.innerHTML=``,x.notes.length===0){let n=document.createElement(`div`);n.style.display=`flex`,n.style.flexWrap=`wrap`,n.style.gap=`0.5rem`,(S===`flat`?t:e).forEach((e,t)=>{let r=document.createElement(`button`);r.type=`button`,r.className=`string-btn`;let a=i(t,4,C);r.innerHTML=`
        <span class="string-name">${e}4</span>
        <span class="string-freq">${a.toFixed(1)} Hz</span>
      `,r.title=`Click to hear ${e}4 reference pitch`,r.addEventListener(`click`,()=>{b.playNote(a,1.2)}),n.appendChild(r)}),F.appendChild(n);return}x.notes.forEach(e=>{let t=document.createElement(`button`);t.type=`button`,t.className=`string-btn`,t.setAttribute(`data-midi`,String(e.midi));let n=i(e.noteIndex,e.octave,C);t.innerHTML=`
      <span class="string-name">${e.label}</span>
      <span class="string-freq">${n.toFixed(1)} Hz</span>
    `,t.title=`Click to hear ${e.label} reference tone`,t.addEventListener(`click`,()=>{b.playNote(n,1.5)}),F.appendChild(t)})}function he(e,t){F.querySelectorAll(`.string-btn`).forEach(n=>{let r=n.getAttribute(`data-midi`);if(!r||e===null){n.classList.remove(`active-target`,`in-tune-string`);return}let i=Number(r);Math.abs(e-i)<=1.2?(n.classList.add(`active-target`),t?n.classList.add(`in-tune-string`):n.classList.remove(`in-tune-string`)):n.classList.remove(`active-target`,`in-tune-string`)})}fe.addEventListener(`change`,()=>{let e=v.find(e=>e.id===fe.value);e&&(x=e,localStorage.setItem(`tunerlab_preset`,e.id),I())}),I();async function L(){if(y.isListening())y.stop(),j.classList.remove(`listening`),M.textContent=`Start Listening`;else{if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){if(window.location.protocol===`http:`&&window.location.hostname!==`localhost`){let e=`https://${window.location.hostname}:5174/`;if(confirm(`Mobile browsers require HTTPS for microphone access.\n\nOpen the HTTPS version at ${e}?`)){window.location.href=e;return}}alert(`Microphone access is not supported over insecure HTTP. Please use https:// on mobile.`);return}try{M.textContent=`Connecting...`,await y.start(),j.classList.add(`listening`),M.textContent=`Microphone Active`}catch{M.textContent=`Mic Error (Click to retry)`,j.classList.remove(`listening`),alert(`Could not access microphone. Please ensure microphone permissions are granted.`)}}}j.addEventListener(`click`,L);var ge=document.querySelector(`#a4-display`),_e=document.querySelector(`#btn-a4-minus`),ve=document.querySelector(`#btn-a4-plus`);function R(e){C=Math.max(415,Math.min(466,e)),ge.textContent=`${C} Hz`,y.setA4(C),A.setA4(C),te.setA4(C),$(),I(),localStorage.setItem(`tunerlab_a4`,String(C))}_e.addEventListener(`click`,()=>R(C-1)),ve.addEventListener(`click`,()=>R(C+1));var ye=document.querySelector(`#notation-select`);ye.addEventListener(`change`,()=>{S=ye.value,y.setNotation(S),A.setNotation(S),I(),localStorage.setItem(`tunerlab_notation`,S)});var z=document.querySelector(`#master-volume`),B=document.querySelector(`#btn-mute`),V=!1,H=.6;z.addEventListener(`input`,()=>{let e=parseFloat(z.value);b.setVolume(e),V=e===0,B.textContent=V?`🔇`:`🔊`}),B.addEventListener(`click`,()=>{V?(V=!1,z.value=String(H||.6),b.setVolume(H||.6),B.textContent=`🔊`):(H=parseFloat(z.value),V=!0,z.value=`0`,b.setVolume(0),B.textContent=`🔇`)});var U=document.querySelector(`#mic-gain-slider`),be=document.querySelector(`#mic-gain-label`),xe=document.querySelector(`#btn-quick-ipad-boost`),Se=document.querySelector(`#noise-gate-slider`),Ce=document.querySelector(`#noise-gate-label`),we=document.querySelector(`#auto-gain-toggle`),Te=document.querySelector(`#raw-audio-toggle`);function W(e){y.setInputGain(e),U.value=String(e),be.textContent=`${e.toFixed(1)}x`,xe.classList.toggle(`active`,e>=3);try{localStorage.setItem(`tunerlab_mic_gain`,String(e))}catch{}}U.addEventListener(`input`,()=>{W(parseFloat(U.value))}),xe.addEventListener(`click`,()=>{W(y.getInputGain()>=3?1:4)}),Se.addEventListener(`input`,()=>{let e=parseFloat(Se.value);y.setNoiseGate(e),Ce.textContent=`${(e*1e3).toFixed(1)}m`;try{localStorage.setItem(`tunerlab_noise_gate`,String(e))}catch{}}),we.addEventListener(`change`,()=>{y.setAutoGainControl(we.checked)}),Te.addEventListener(`change`,()=>{y.setRawAudioMode(Te.checked)});var Ee=document.querySelectorAll(`.viz-toggle-btn`),De=document.querySelector(`#viz-title`);Ee.forEach(e=>{e.addEventListener(`click`,()=>{Ee.forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`);let t=e.getAttribute(`data-viz`);k.setMode(t),De.textContent=t===`waveform`?`Live Audio Waveform`:`Pitch Stability Trace`})});var Oe=document.querySelectorAll(`.tab-btn`),ke=document.querySelectorAll(`.tab-panel`),G=`tuner`;function K(e){G=e,Oe.forEach(t=>{let n=t.getAttribute(`data-tab`)===e;t.classList.toggle(`active`,n),t.setAttribute(`aria-selected`,String(n))}),ke.forEach(t=>{let n=t.id===`panel-${e}`;t.classList.toggle(`active`,n)}),e===`tuner`&&setTimeout(()=>{O.handleResize(),k.handleResize()},50)}Oe.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-tab`);t&&K(t)})});var q=document.querySelector(`#btn-play-tone`),J=document.querySelector(`#btn-drone-toggle`),Y=document.querySelectorAll(`.note-grid-btn`),X=document.querySelector(`#gen-oct-display`),Ae=document.querySelector(`#btn-gen-oct-minus`),je=document.querySelector(`#btn-gen-oct-plus`),Z=document.querySelector(`#gen-freq-display`),Q=document.querySelector(`#gen-freq-slider`),Me=document.querySelectorAll(`.timbre-btn`);function $(){E=i(w,T,C),Z.textContent=`${E.toFixed(1)} Hz`,Q.value=String(E),q.innerHTML=`<span>🔊</span> Play Note (${e[w]}${T} &bull; ${E.toFixed(1)} Hz)`,b.isDronePlaying()&&b.updateDroneFrequency(E)}Y.forEach(e=>{e.addEventListener(`click`,()=>{Y.forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),w=Number(e.getAttribute(`data-idx`)),$()})}),Ae.addEventListener(`click`,()=>{T>1&&(T--,X.textContent=String(T),$())}),je.addEventListener(`click`,()=>{T<7&&(T++,X.textContent=String(T),$())}),Q.addEventListener(`input`,()=>{E=parseFloat(Q.value),Z.textContent=`${E.toFixed(1)} Hz`,q.innerHTML=`<span>🔊</span> Play Custom (${E.toFixed(1)} Hz)`,b.isDronePlaying()&&b.updateDroneFrequency(E)}),Me.forEach(e=>{e.addEventListener(`click`,()=>{Me.forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),D=e.getAttribute(`data-timbre`),b.setTimbre(D)})}),q.addEventListener(`click`,()=>{b.playNote(E,1.5,D)}),J.addEventListener(`click`,()=>{b.isDronePlaying()?(b.stopDrone(),J.classList.remove(`active`),J.innerHTML=`<span>〰️</span> Start Continuous Drone`):(b.startDrone(E,D),J.classList.add(`active`),J.innerHTML=`<span>⏹️</span> Stop Continuous Drone`)}),A.setOnNoteSelect((t,n,r)=>{E=t,T=r;let i=e.indexOf(n);i!==-1&&(w=i),X.textContent=String(T),Z.textContent=`${E.toFixed(1)} Hz`,Q.value=String(E),Y.forEach(e=>{e.classList.toggle(`active`,Number(e.getAttribute(`data-idx`))===w)}),q.innerHTML=`<span>🔊</span> Play Note (${n}${r} &bull; ${E.toFixed(1)} Hz)`}),window.addEventListener(`keydown`,e=>{e.target instanceof HTMLInputElement||e.target instanceof HTMLSelectElement||G===`piano`&&A.handleKeyDown(e)||(e.code===`Space`?(e.preventDefault(),L()):e.key===`1`?K(`tuner`):e.key===`2`?K(`generator`):e.key===`3`?K(`piano`):e.key===`4`?K(`trainer`):e.key.toLowerCase()===`d`&&G!==`piano`?J.click():e.key.toLowerCase()===`m`&&G!==`piano`&&B.click())}),window.addEventListener(`keyup`,e=>{G===`piano`&&A.handleKeyUp(e)});